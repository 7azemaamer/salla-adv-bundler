...
// Helper function to minify JavaScript (basic minification)
...
router.get("/modal.js", (req, res) => {
  const shouldMinify = process.env.NODE_ENV === 'production' || req.query.min === 'true';
  
  res.set({
    "Content-Type": "application/javascript",
    // Enable aggressive caching with version-based cache busting
...
`;
  ...
  // Optionally minify the script
  const finalScript = shouldMinify ? minifyJS(modalScript) : modalScript;
  
  res.send(finalScript);
});

// Serve the CSS file with caching
router.get("/modal.css", (req, res) => {
  res.set({
  ...
  res.sendFile('bundle-modal.css', { root: './public' });
});
